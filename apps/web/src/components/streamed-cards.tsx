...
export function CourseCard({ headline, summary, facts, nextSteps, tone }: CourseCardProps) {
  ...
  return (
    <article className="ck-card" style={{ borderLeftColor: color }}>
      <h3>{headline || "Preparing course summary…"}</h3>
      <p>{summary || "Gathering course details…"}</p>
      {!!facts?.length && (
